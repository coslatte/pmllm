...
| `EMBEDDING_API_URL`             | `http://localhost:8081/v1/embeddings`       | URL (vista desde el host) para solicitar embeddings (contenedor gemma-embeddings).            |
...
| `LLM_API_URL`                   | `http://localhost:8082/v1/chat/completions` | URL (vista desde el host) para solicitudes de chat (contenedor gemma-chat).                   |
...
> **Recordatorio de contenedores:** Los modelos Gemma (embeddings y chat) se ejecutan en contenedores separados (`gemma-embeddings` y `gemma-chat`). Mantén estos servicios activos durante las construcciones y consultas. Si llamas desde otro contenedor en la misma red Docker, usa `http://gemma-embeddings:8080/...` o `http://gemma-chat:8080/...` en las variables `_API_URL`.
